...
const getAllPromotionByDay = async (con, result1) => {
  return new Promise((resolve, reject) => {
    con.query(
      `SELECT pp.code, pp.day FROM promotion_by_day AS pp, promotion AS p WHERE p.code = pp.code`,
      (err, result2) => {
        if (err) throw err;
...
const getOnePricePerTypeP = async (con, result1, code) => {
  return new Promise((resolve, reject) => {
    con.query(
      `SELECT * FROM price_per_type_p WHERE code = "${code}"`,
      (err, result3) => {
        if (err) throw err;
...
const getOnePromotionByDay = async (con, result1, code) => {
  return new Promise((resolve, reject) => {
    con.query(
      `SELECT pp.code, pp.day FROM promotion_by_day AS pp, promotion AS p WHERE p.code = pp.code AND p.code = "${code}"`,
      (err, result2) => {
        if (err) throw err;
  ...
  con.connect(async (err) => {
    ...
    con.query(sql1, async (err, result1) => {
      ...
      if (result1[0] == undefined) {
        res
          .status(200)
          .send({ msg: "OK", isPromotion: false, data: result1[0] });
        ...
        res
          .status(200)
          .send({ msg: "OK", isPromotion: true, data: result1[0] });
